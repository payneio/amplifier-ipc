...
def content_files() -> list[str]:
    """Scan amplifier_design_intelligence content once for the entire test module."""
    return scan_content("amplifier_design_intelligence")
...
def test_design_instructions_present(content_files: list[str]) -> None:
    """context/design-instructions.md must be present in discovered content."""
    assert "context/design-instructions.md" in content_files, (
        f"Expected 'context/design-instructions.md' in content files. "
        f"Found context files: {_files_in_dir(content_files, 'context')}"
    )


def test_knowledge_base_present(content_files: list[str]) -> None:
    """context/knowledge-base/accessibility.md must be present in discovered content."""
    assert "context/knowledge-base/accessibility.md" in content_files, (
        f"Expected 'context/knowledge-base/accessibility.md' in content files. "
        f"Found context files: {_files_in_dir(content_files, 'context')}"
    )


def test_design_philosophy_present(content_files: list[str]) -> None:
    """context/philosophy/DESIGN-PHILOSOPHY.md must be present in discovered content."""
    assert "context/philosophy/DESIGN-PHILOSOPHY.md" in content_files, (
        f"Expected 'context/philosophy/DESIGN-PHILOSOPHY.md' in content files. "
        f"Found context files: {_files_in_dir(content_files, 'context')}"
    )
