...
def content_files() -> list[str]:
    """Scan amplifier_recipes content once for the entire test module."""
    return scan_content("amplifier_recipes")
...
def test_recipe_instructions_present(content_files: list[str]) -> None:
    """context/recipe-instructions.md must be present in discovered content."""
    assert "context/recipe-instructions.md" in content_files, (
        f"Expected 'context/recipe-instructions.md' in content files. "
...
def test_validate_recipes_present(content_files: list[str]) -> None:
    """recipes/validate-recipes.yaml must be present in discovered content."""
    assert "recipes/validate-recipes.yaml" in content_files, (
        f"Expected 'recipes/validate-recipes.yaml' in content files. "
