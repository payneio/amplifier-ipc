...
def test_version_attribute() -> None:
    """amplifier_amplifier.__version__ must equal '0.1.0'."""
    version = importlib.metadata.version("amplifier-amplifier")
    assert version == "0.1.0"


def test_package_docstring() -> None:
    """amplifier_amplifier must have a non-empty module docstring."""
    mod = importlib.import_module("amplifier_amplifier")
    assert mod.__doc__ is not None
    assert len(mod.__doc__.strip()) > 0


def test_main_module_exists() -> None:
    """src/amplifier_amplifier/__main__.py must exist."""
    main_path = PROJECT_ROOT / "src" / "amplifier_amplifier" / "__main__.py"
    assert main_path.exists(), f"__main__.py not found at {main_path}"


def test_main_has_main_function() -> None:
    """amplifier_amplifier.__main__ must export a main() function."""
    mod = importlib.import_module("amplifier_amplifier.__main__")
    assert hasattr(mod, "main"), "__main__.py must define a main() function"
    assert callable(mod.main)


def test_context_directory_exists() -> None:
    """src/amplifier_amplifier/context/ directory must exist."""
    context_path = PROJECT_ROOT / "src" / "amplifier_amplifier" / "context"
    assert context_path.is_dir(), f"context/ directory not found at {context_path}"


def test_recipes_directory_exists() -> None:
    """src/amplifier_amplifier/recipes/ directory must exist."""
    recipes_path = PROJECT_ROOT / "src" / "amplifier_amplifier" / "recipes"
    assert recipes_path.is_dir(), f"recipes/ directory not found at {recipes_path}"


def test_entry_point_available() -> None:
    """amplifier-amplifier-serve must be installed as a console script."""
    bin_dir = Path(sys.executable).parent
    serve_script = bin_dir / "amplifier-amplifier-serve"
