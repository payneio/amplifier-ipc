...
def content_files() -> list[str]:
    """Scan amplifier_browser_tester content once for the entire test module."""
    return scan_content("amplifier_browser_tester")
...
def test_browser_guide_present(content_files: list[str]) -> None:
    """context/browser-guide.md must be present in discovered content."""
    assert "context/browser-guide.md" in content_files, (
        f"Expected 'context/browser-guide.md' in content files. "
...
def test_visual_audit_recipe_present(content_files: list[str]) -> None:
    """recipes/visual-audit.yaml must be present in discovered content."""
    assert "recipes/visual-audit.yaml" in content_files, (
        f"Expected 'recipes/visual-audit.yaml' in content files. "
