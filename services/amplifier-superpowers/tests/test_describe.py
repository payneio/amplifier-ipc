...
def test_discovers_no_tools() -> None:
    """amplifier_superpowers must report no tools (content-only service)."""
    components = scan_package("amplifier_superpowers")
...
def test_discovers_no_hooks() -> None:
    """amplifier_superpowers must report no hooks (content-only service)."""
    components = scan_package("amplifier_superpowers")
...
def test_discovers_no_orchestrators() -> None:
    """amplifier_superpowers must report no orchestrators (content-only service)."""
    components = scan_package("amplifier_superpowers")
...
def test_discovers_no_context_managers() -> None:
    """amplifier_superpowers must report no context_managers (content-only service)."""
    components = scan_package("amplifier_superpowers")
...
def test_discovers_no_providers() -> None:
    """amplifier_superpowers must report no providers (content-only service)."""
    components = scan_package("amplifier_superpowers")
...
def test_discovers_content() -> None:
    """amplifier_superpowers must expose at least 1 content file via scan_content."""
    paths = scan_content("amplifier_superpowers")
    assert len(paths) > 0, "Expected at least one content file, got none"


def test_content_includes_behaviors_files() -> None:
    """scan_content must include files from the behaviors/ directory."""
    paths = scan_content("amplifier_superpowers")
    behavior_files = [p for p in paths if p.startswith("behaviors/")]
    assert len(behavior_files) >= 1, (
        f"Expected >= 1 behavior file, got: {behavior_files}"
    )
