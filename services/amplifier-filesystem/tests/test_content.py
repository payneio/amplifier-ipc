...
def content_files() -> list[str]:
    """Scan amplifier_filesystem content once for the entire test module."""
    return scan_content("amplifier_filesystem")
...
def test_editing_guidance_present(content_files: list[str]) -> None:
    """context/editing-guidance.md must be present in discovered content."""
    assert "context/editing-guidance.md" in content_files, (
        f"Expected 'context/editing-guidance.md' in content files. "
